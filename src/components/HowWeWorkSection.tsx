...
const iconMap: Record<string, any> = { Search, Lightbulb, Rocket, BarChart3 };
...
const HowWeWorkSection = () => {
  const { data: dbContent } = usePageContent("/", "how-we-work");

  const content = dbContent as Record<string, any> | null;
  const badge = content?.badge || "Our Process";
  const heading = content?.heading || "How We";
  const headingHighlight = content?.headingHighlight || "Deliver Results";
  const subtitle = content?.subtitle || "A proven 4-step framework that transforms businesses through strategic digital marketing.";
  const steps = content?.steps && Array.isArray(content.steps) ? content.steps : staticSteps;
  const ctaText = content?.ctaText || "Start Your Growth Journey";
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {steps.map((step: any, index: number) => {
            ...
            const color = step.color || "from-blue-500 to-cyan-500";
            return (
              <AnimatedSection key={step.step} delay={index * 0.15}>
                ...
                >
                  <motion.div
                    className={`absolute -top-3 -right-3 w-10 h-10 rounded-full bg-gradient-to-br ${color} flex items-center justify-center text-sm font-bold text-white shadow-lg`}
                  ...
                  <motion.div
                    className={`w-14 h-14 rounded-xl bg-gradient-to-br ${color} flex items-center justify-center mb-5 shadow-lg`}
                  ...
                  <div className={`inline-flex items-center self-start px-2.5 py-1 rounded-full text-xs font-medium mb-3 bg-gradient-to-r ${color} text-white/90`}>
                    ⏱ {step.duration}
                  </div>

                  <div className="flex flex-wrap gap-1.5">
                    {(step.highlights || []).map((h: string) => (
                  ...
                  <motion.div className={`absolute bottom-0 left-0 h-0.5 bg-gradient-to-r ${color} rounded-b-2xl`} initial={{ width: 0 }} whileInView={{ width: "100%" }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.3 + index * 0.15 }} />
                </motion.div>
              </AnimatedSection>
            );
          })}
