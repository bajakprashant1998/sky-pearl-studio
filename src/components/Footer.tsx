...
const Footer = () => {
  ...
  const socials = [
    { icon: Facebook, href: "https://www.facebook.com/share/1GGViEsE5a/?mibextid=wwXIfr", label: "Facebook", color: "hover:bg-[#1877F2]" },
    { icon: Twitter, href: "https://x.com/digital_1221?s=21&t=gZqAEY-otu1upyCHIOj4Uw", label: "Twitter", color: "hover:bg-[#1DA1F2]" },
    { icon: Linkedin, href: "https://www.linkedin.com/company/digitalbulltechnology/", label: "LinkedIn", color: "hover:bg-[#0A66C2]" },
    ...
    <>
      <Helmet>
        <script type="application/ld+json">
          {JSON.stringify(globalFaqSchema)}
        </script>
      </Helmet>
            ...
            <div className="flex gap-3">
              {socials.map((social, index) => (
                <motion.a
                  ...
                  className={`w-11 h-11 bg-slate-800/80 rounded-xl flex items-center justify-center text-slate-400 hover:text-white transition-all duration-300 border border-slate-700/50 hover:border-transparent hover:shadow-lg ${social.color}`}
                ...
                {servicesCol1.map((service) => (
                  <li key={service.id}>
                    <Link
                      ...
                      className="text-slate-400 hover:text-blue-400 transition-all text-sm flex items-center gap-2 group"
                    >
              ...
              <ul className="space-y-3">
                {links.legal.map((link) => (
                  <li key={link.name}>
                    <Link
                      to={link.href}
                      className="text-slate-400 hover:text-purple-400 transition-all text-sm flex items-center gap-2 group"
                    >
                      <ArrowRight className="w-3 h-3 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all" />
                      <span className="group-hover:translate-x-1 transition-transform">{link.name}</span>
                    </Link>
                  ...
                  <div className="grid grid-cols-1 gap-2 pb-4">
                    {services.map((service) => (
                      <Link
                        key={service.id}
                        to={`/services/${service.slug}`}
                        className="text-slate-400 hover:text-blue-400 transition-colors py-2 block text-sm pl-4 border-l-2 border-slate-800 hover:border-blue-500"
                      >
                        {service.shortTitle}
                      </Link>
                    ))}
                  ...
                  <ul className="space-y-2 pb-4">
                    {links.company.map((link) => (
                      <li key={link.name}>
                        <Link
                          to={link.href}
                          className="text-slate-400 hover:text-green-400 transition-colors text-sm block py-2 pl-4 border-l-2 border-slate-800 hover:border-green-500"
                        >
                          {link.name}
                        </Link>
                      </li>
                  ...
                  <ul className="space-y-2 pb-4">
                    {links.legal.map((link) => (
                      <li key={link.name}>
                        <Link
                          to={link.href}
                          className="text-slate-400 hover:text-purple-400 transition-colors text-sm block py-2 pl-4 border-l-2 border-slate-800 hover:border-purple-500"
                        >
                          {link.name}
                        </Link>
                      </li>
        ...
        <div className="pt-8 border-t border-slate-800/50">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex items-center gap-3 text-slate-600 text-xs">
              <span className="px-2 py-1 bg-slate-800/50 rounded">Google Partner</span>
              <span className="px-2 py-1 bg-slate-800/50 rounded">Meta Business Partner</span>
              <span className="px-2 py-1 bg-slate-800/50 rounded hidden sm:inline">Microsoft Partner</span>
            </div>
          </div>
        </div>
      </div>
