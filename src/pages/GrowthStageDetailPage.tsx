...
const StageBreadcrumb = ({ stageName }: { stageName: string }) => (
...
const StageHero = ({ stage }: { stage: ReturnType<typeof getStageBySlug> }) => {
...
const StageOverview = ({ stage }: { stage: ReturnType<typeof getStageBySlug> }) => {
  if (!stage) return null;
  return (
...
const StageSteps = ({ stage }: { stage: ReturnType<typeof getStageBySlug> }) => {
  if (!stage) return null;
  return (
...
const StageExamples = ({ stage }: { stage: ReturnType<typeof getStageBySlug> }) => {
  if (!stage) return null;
  return (
    <section className="py-20 lg:py-28 bg-background">
...
const StageServicesOutcomes = ({ stage }: { stage: ReturnType<typeof getStageBySlug> }) => {
  if (!stage) return null;
  return (
    <section className="py-20 lg:py-28 bg-muted/30">
      <div className="container mx-auto px-4 max-w-5xl">
...
const StageFAQs = ({ stage }: { stage: ReturnType<typeof getStageBySlug> }) => {
...
const StageNavigation = ({ stage }: { stage: ReturnType<typeof getStageBySlug> }) => {
  if (!stage) return null;
...
const AllStagesOverview = ({ currentStageNumber }: { currentStageNumber: number }) => (
  <section className="py-20 lg:py-28 bg-background border-t border-border">
    <div className="container mx-auto px-4 max-w-5xl">
      <AnimatedSection direction="up" className="text-center mb-12">
        <Badge variant="outline" className="mb-4 text-xs uppercase tracking-widest">The Complete Journey</Badge>
        <h2 className="text-3xl font-bold text-foreground">All 5 Growth Stages</h2>
        <p className="text-muted-foreground mt-3 max-w-xl mx-auto">See where this stage fits in the bigger picture.</p>
      </AnimatedSection>
      ...
      <div className="grid sm:grid-cols-5 gap-4">
        {growthStages.map((s) => {
          ...
          return (
            <AnimatedSection key={s.number} direction="up" delay={s.number * 0.05}>
              <Link to={`/growth-strategy/${s.slug}`}>
                <motion.div
                  className={`rounded-xl p-5 text-center border transition-all h-full ${
                    isCurrent
                  ...
                  whileHover={{ y: -3 }}
                >
                  <div className={`w-10 h-10 mx-auto rounded-lg bg-gradient-to-br ${s.accent} flex items-center justify-center mb-3`}>
                    <s.icon className="w-5 h-5 text-white" />
                  </div>
                  <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">Stage {s.number}</p>
                  <h3 className={`text-sm font-bold ${isCurrent ? "text-primary" : "text-foreground"}`}>{s.name}</h3>
                  {isCurrent && <Badge className="mt-2 text-[10px]">You're Here</Badge>}
                </motion.div>
              </Link>
            </AnimatedSection>
          );
...
const GrowthStageDetailPage = () => {
  ...
  const jsonLd = {
    ...
    headline: `${stage.name} — Growth Strategy Stage ${stage.number}`,
    description: stage.objective,
    url: `https://dibull.com/growth-strategy/${stage.slug}`,
    publisher: {
      "@type": "Organization",
      name: "Digital Bull",
    ...
    isPartOf: {
      ...
      url: "https://dibull.com/growth-strategy",
    },
  };
  ...
  return (
    <>
      <Helmet>
        ...
        <meta property="og:title" content={`${stage.name} — Growth Strategy | Digital Bull`} />
        <meta property="og:description" content={stage.objective} />
        <meta property="og:type" content="article" />
        <meta property="og:url" content={`https://dibull.com/growth-strategy/${stage.slug}`} />
        <meta property="og:image" content="https://dibull.com/dibull_logo.png" />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={`${stage.name} — Growth Strategy | Digital Bull`} />
        <meta name="twitter:description" content={stage.objective} />
      ...
      <main className="pt-20">
        ...
        <StageExamples stage={stage} />
        <StageServicesOutcomes stage={stage} />
        <StageFAQs stage={stage} />
        <AllStagesOverview currentStageNumber={stage.number} />
